		...
		<?php
			if(isset($_POST['Submit']))
			{
				...
				if(empty($data_missing))
				{
					...
					if($cnt==1)
					{
						$query="INSERT INTO Flight_Details (flight_no,from_city,to_city,departure_date,arrival_date,departure_time,arrival_time,seats_economy,seats_business,price_economy,price_business,jet_id,jet_type) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
						$stmt=mysqli_prepare($dbc,$query);
						mysqli_stmt_bind_param($stmt,"sssssssiiiiss",$flight_no,$origin,$destination,$dep_date,$arr_date,$dep_time,$arr_time,$seats_eco,$seats_bus,$price_eco,$price_bus,$jet_id,$jet_type);
						mysqli_stmt_execute($stmt);
